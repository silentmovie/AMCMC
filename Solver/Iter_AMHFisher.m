function [rhohist,psihist, Ham, effSteps,alphathist]=Iter_AMHFisher(pai,rho0,Q,psi0,alphat,tspan,deltaT,mode)
...
for j=2:(TotIt+1)
    ...
    %            );              %omega(kCur) is the large quotient in eq:eQ1
    % psiCur(abs(psiCur)>15) = log(kCur(abs(psiCur)>15));
